...
**Config publicável.** `config/arqel-realtime.php` expõe cinco blocos: `connection` (alias do broadcaster, fallback para `broadcasting.default`), `channel_prefix` (default `arqel`), `auto_dispatch.resource_updated` (default `true`, kill switch global), `presence` com `enabled` (env `ARQEL_REALTIME_PRESENCE_ENABLED`, default `true`) + `channel_pattern` (placeholders `{resource}` e `{recordId}`), e `workflow.broadcast_state_transitions` (env `ARQEL_REALTIME_WORKFLOW_BROADCAST`, default `true`, rebroadcast de transições de state quando `arqel-dev/workflow` está instalado).
...
| `Http\Controllers\CollabDocumentController` (final) | Métodos `show()` / `store()` para `GET|POST /admin/{resource}/{id}/collab/{field}`. **Autorização por registro** (#103): cada request resolve o model dono a partir de `{resource}` (slug via `ResourceRegistry::findBySlug()` ou FQCN direto) + `{id}` e autoriza `view` (show) / `update` (store) antes de ler/escrever — espelha o `AwarenessChannelAuthorizer` (mesma resolução + padrão Policy-aware `Gate::has() || Gate::getPolicyFor()`, scaffold mode quando não há gate nem policy). `abort(404)` quando o model/registro não resolve; `403` quando a ability é negada. Optimistic concurrency via `version`: requests com `version < current` recebem `409 {message, serverVersion}`. State sempre transitado em base64. |
| `routes/api.php` | Registra os endpoints com middleware `web,auth`. |
| `Collab\AwarenessChannelAuthorizer` (final readonly) | Autoriza o canal `arqel.collab.{modelType}.{modelId}.{field}`. Resolve o model via FQCN direto, via `ResourceRegistry::all()` (match por `getModel()`) **ou via `ResourceRegistry::findBySlug()`** (#116) — espelha o `CollabDocumentController::resolveModelClass`, de modo que o `{modelType}` aceito é exatamente a chave (slug) que o REST persiste e na qual o `YjsUpdateReceived` broadcasta. Defensive: denega quando registry unbound + nem FQCN nem slug resolvem. Aplica `view` quando a app define um Gate `view` **OU** registra uma Policy para o model (via `Gate::getPolicyFor()`, já que `Gate::has()` nunca enxerga Policies — ver #92); só faz allow quando não há Gate **nem** Policy (scaffold mode). |
...
Quando `arqel-dev/workflow` está presente no autoload, o `RealtimeServiceProvider` registra automaticamente o listener `Workflow\BroadcastStateTransitionListener` para o evento `Arqel\Workflow\Events\StateTransitioned`. A cada transição de state bem-sucedida, o listener despacha um `Events\ResourceUpdated` carregando o mesmo `record` + `userId` do evento de origem. O listener resolve a **classe Resource** do model via `ResourceRegistry::findByModel($record::class)` e a passa como `resourceClass` (#103) — assim `ResourceUpdated::resolveSlug()` lê o `$slug` registrado da Resource em vez de cair no fallback de `class_basename`, garantindo que os canais `arqel.{slug}` + `arqel.{slug}.{id}` batam com os canais que a UI já assina. Quando o registry está unbound (realtime standalone) ou não tem Resource para o model, o listener faz fallback para o FQCN do model, preservando o comportamento anterior. Toda UI já assinante de um Resource cuja Eloquent model use `HasWorkflow` recebe a notificação sem código adicional.
